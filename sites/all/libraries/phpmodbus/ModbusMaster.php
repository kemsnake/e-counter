...
class ModbusMaster {
  ...
  /**
   * writeMultipleRegisterCustom
   *
   * Получаем данные после записи, в остальном - writeMultipleRegisterCustom
   *
   */
  function writeMultipleRegisterCustom($unitId, $reference, $data, $dataTypes){
    $this->status .= "writeMultipleRegister: START\n";
    // connect
    $this->connect();
    // send FC16
    ...
    // disconnect
    //$receivedData = $this->readMultipleRegistersParser($rpacket);
    $receivedData = $this->readWriteRegistersParserCustom($rpacket);
    $this->disconnect();
    $this->status .= "writeMultipleRegister: DONE\n";
    return $receivedData;
  }
   ...
   * @return array
   */
  private function readWriteRegistersParserCustom($packet){
    $data = array();
    // if not exception
    if(!$this->responseCode($packet))
      return false;
    // todo используем 576 - 12 страниц архива логики-941. переделать в параметр
    try{
      for($i=0;$i<576;$i++){
        $data[$i] = ord($packet[9+$i]);
      }
